...
template <std::floating_point T> void VolSurface<T>::setDiscountFactors_() noexcept
{
...
template <std::floating_point T> void VolSurface<T>::setForwards_() noexcept
{
    for (std::size_t i = 0; i < numTenors_; ++i)
    {
...
template <std::floating_point T> void VolSurface<T>::setCallPrices_()
{
...
template <std::floating_point T> void VolSurface<T>::setLogKFMatrix_()
{
...
{
    UV_REQUIRE(
        totVarMatrix.rows() == numTenors_ && totVarMatrix.cols() == numStrikes_,
...
{
    UV_REQUIRE(
        callPrices.rows() == numTenors_ && callPrices.cols() == numStrikes_,
        ErrorCode::InvalidArgument,
...
template <std::floating_point T> const Vector<T>& VolSurface<T>::tenors() const noexcept
{
    return tenors_;
}

template <std::floating_point T> std::size_t VolSurface<T>::numTenors() const noexcept
{
    return numTenors_;
}

template <std::floating_point T> const Vector<T>& VolSurface<T>::mny() const noexcept
{
    return mny_;
}
...
template <std::floating_point T> T VolSurface<T>::S() const noexcept
{
    return S_;
}

template <std::floating_point T> const Vector<T>& VolSurface<T>::rates() const noexcept
{
    return rates_;
}
...
template <std::floating_point T> const Vector<T>& VolSurface<T>::strikes() const noexcept
{
    return strikes_;
}

template <std::floating_point T> const Vector<T>& VolSurface<T>::forwards() const noexcept
{
    return forwards_;
}
...
template <std::floating_point T>
void VolSurface<T>::printVol(const unsigned int valuePrec, const bool mnyFlag)
    const noexcept
{
    const std::span<const T> header{mnyFlag ? std::span<const T>(mny_) : logKFMatrix_[0]};
...
template <std::floating_point T>
void VolSurface<T>::printTotVar(const unsigned int valuePrec, const bool mnyFlag)
    const noexcept
{
    const std::span<const T> header{mnyFlag ? std::span<const T>(mny_) : logKFMatrix_[0]};
    const int headerPrec{mnyFlag ? 2 : 4};
...
template <std::floating_point T>
void VolSurface<T>::printBSCall(const unsigned int valuePrec, const bool mnyFlag)
    const noexcept
{
    const std::span<const T> header{mnyFlag ? std::span<const T>(mny_) : logKFMatrix_[0]};
    const int headerPrec{mnyFlag ? 2 : 4};
