...
template <std::floating_point T> T d1BlackScholes(T t, T r, T q, T vol, T S, T K) noexcept
{
    return std::fma(t, (r - q + vol * vol * T(0.5)), std::log(S / K)) /
           (std::sqrt(t) * vol);
}

template <std::floating_point T>
T blackScholes(T t, T r, T q, T vol, T S, T K, bool isCall) noexcept
{
...
template <std::floating_point T> T vegaBS(T d1, T t, T q, T S) noexcept
{
    return S * std::exp(-q * t) * normalPDF(d1) * std::sqrt(t);
}

template <std::floating_point T> T volgaBS(T vega, T d1, T t, T vol) noexcept
{
    T d2{std::fma(-vol, std::sqrt(t), d1)};
    return vega * (d1 * d2) / vol;
}

template <std::floating_point T> T impliedVolBS(T callPrice, T t, T r, T q, T S, T K)
{
    ...
    UV_WARN(
        vol != volGuess,
        std::format(
            "impliedVolBS: initial guess = {:.6f} clamped to {:.6f} (lb = "
            "{:.4f}, ub = {:.4f})",
            volGuess,
            vol,
            volMin,
    ...
    for (unsigned int i = 0U; i < EVAL; ++i)
    {
        ...
        if (std::fabs(objEval) < TOL * (T(1) + priceBS))
            break;
    ...
    UV_WARN(
        std::fabs(finalResidual) > finalTol,
        std::format(
            "impliedVolBS: no convergence after {} iterations "
            "(|f| = {:.3e} > tol = {:.3e}, vol = {:.6f})",
            EVAL,
            std::fabs(finalResidual),
            finalTol,
            vol
        )
    );
