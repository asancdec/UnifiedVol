		...
		std::span<const T> ys)
	{
		...
		for (std::size_t i = 0; i < xsSize; ++i)
		{
			UV_REQUIRE(
				std::isfinite(xs[i]),
				ErrorCode::InvalidArgument,
				"validateInputs: xs[" + std::to_string(i) + "] = " +
				std::to_string(xs[i]) + " is not finite"
			);

			UV_REQUIRE(
				std::isfinite(ys[i]),
				ErrorCode::InvalidArgument,
				"validateInputs: ys[" + std::to_string(i) + "] = " +
				std::to_string(ys[i]) + " is not finite"
			);
		}
		...
		UV_REQUIRE(
			...
			"(violation at index " + std::to_string(violIndex) +
