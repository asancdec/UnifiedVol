    ...
    for (std::size_t i = 0; i < initGuess.size(); ++i)
    {
        ...
        UV_WARN(after != before,
            std::format("Calibrator: parameter '{}' initial guess = {:.4f} "
                ...
                "(lb = {:.4f}, ub = {:.4f})",
                config_.paramNames_[i], before, after,
...
template <std::size_t N>
double Calibrator<N>::ObjectiveThunk(unsigned n, const double* x, double* grad, void* p) noexcept
{
...
template <std::size_t N>
void Calibrator<N>::addInequalityConstraint(
    NloptFunction c,
    void* data) noexcept
{
...
template <std::size_t N>
void Calibrator<N>::setMinObjective(NloptFunction f, void* data) noexcept
{
    iterCount_ = 0;
    objCtx_.fn = f;
    objCtx_.user = data;
    objCtx_.iter = &iterCount_;
    opt_.set_min_objective(&Calibrator<N>::ObjectiveThunk, &objCtx_);
}

template <std::size_t N>
std::vector<double> Calibrator<N>::optimize() noexcept
{
    // Start time
