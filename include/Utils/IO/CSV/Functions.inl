...
readVolSurface(const std::string& filename, const core::MarketData<T>& mktData)
{
    ...
    auto trim = [](std::string& s)
    {
        ...
        while (!s.empty() && (issp(static_cast<unsigned char>(s.front())) != 0))
            s.erase(s.begin());
        while (!s.empty() && (issp(static_cast<unsigned char>(s.back())) != 0))
            s.pop_back();
    };
    ...
    auto parseCell = [&](const std::string& raw, T& out, bool strict = true) -> bool
    {
        ...
        if (!s.empty() && s.back() == '%')
        {
            ...
            {
                if (strict)
                    raise(ErrorCode::DataFormat, "Lonely % cell");
                else
                    return false;
            }
        }
        ...
        {
            if (strict)
                raise(ErrorCode::DataFormat, "Non-numeric cell: \"" + raw + "\"");
            else
                return false;
        }
        ...
        {
            if (strict)
                raise(ErrorCode::DataFormat, "Trailing junk in \"" + raw + "\"");
            return false;
        }
    ...
    auto splitComma = [](const std::string& line)
    {
        ...
        if (!line.empty() && line.back() == ',')
            cells.emplace_back(""); // trailing comma
        return cells;
    };
    ...
    {
        raise(
            ErrorCode::DataFormat,
            "Header must have at least 2 columns (blank/label + one "
            "moneyness)"
        );
    }
    ...
    for (std::size_t j = 1; j < header.size(); ++j) // skip first header cell
    {
        ...
        if (!parseCell(header[j], v, /*strict=*/true))
            raise(
                ErrorCode::DataFormat,
                "Non-numeric moneyness at header col " + std::to_string(j + 1)
            );
        mny.push_back(v);
    }
    ...
    while (std::getline(file, line))
    {
        ...
        if (cells.size() < 2)
        {
            raise(
                ErrorCode::DataFormat,
                "Row " + std::to_string(lineNo) + " has fewer than 2 columns"
            );
        }
        ...
        if (!parseCell(cells[0], t, /*strict=*/true))
        {
            raise(
                ErrorCode::DataFormat,
                "Missing/invalid maturity at row " + std::to_string(lineNo)
            );
        }
        ...
        for (std::size_t j = 0; j < mny.size(); ++j)
        {
            ...
            if (!parseCell(cells[1 + j], sigma, /*strict=*/true))
            {
                raise(
                    ErrorCode::DataFormat,
                    "Non-numeric vol at row " + std::to_string(lineNo) + ", col " +
                        std::to_string(1 + j + 1)
                );
            }
            row.push_back(sigma);
    ...
    for (std::size_t i = 0; i < vols.size(); ++i)
    {
        if (vols[i].size() != mny.size())
        {
            raise(
                ErrorCode::DataFormat,
                "Ragged row at data row " +
                    std::to_string(i + 2) + // +2 for header + 1-based
                    ": got " + std::to_string(vols[i].size()) + " vols, expected " +
                    std::to_string(mny.size())
            );
        }
    }
