...
namespace uv::models::heston
{
	...
	template <std::floating_point T, std::size_t N>
	T Pricer<T, N>::callPrice(T kappa,
		T theta,
		T sigma,
		T rho,
		T v0,
		...
		auto integrand = [=](T x) noexcept -> T
			{
				...
				const Complex<T> psi{ Pricer<T, N>::charFunction(kappa, theta, sigma, rho, v0, t, hMinusI) };
	...
	template <std::floating_point T, std::size_t N>
	std::array<T, 6> Pricer<T, N>::callPriceWithGradient(T kappa,
		T theta,
		T sigma,
		T rho,
		T v0,
		...
		auto batchIntegrand = [=](T x) noexcept -> std::array<T, 6>
			{
				...
				// g' helper
				const auto dg_from = [&D, &denomG, &beta](const Complex<T>& dbeta, const Complex<T>& dD) noexcept -> Complex<T>
					{
				...
				// B' helper
				const auto dB_from = [&](const Complex<T>& dbeta, const Complex<T>& dD, T dC) noexcept -> Complex<T>
					{
				...
				// S' helper
				const auto dS_from = [&](const Complex<T>& dbeta, const Complex<T>& dD, const Complex<T>& dg) noexcept -> Complex<T>
					{
	...
	template <std::floating_point T, std::size_t N>
	T Pricer<T, N>::getResidues(T alpha,
		const T F,
		const T K) noexcept
	{
		if (alpha < -T(1)) return F - K;
		else return T(0.0);
	}
	...
	{
		if (w >= T(0.0)) return config_.alphaItm;
		else return config_.alphaOtm;
	}

	template <std::floating_point T, std::size_t N>
	T Pricer<T, N>::getPhi(T kappa,
		T theta,
		T sigma,
		T rho,
		T v0,
		...
		T w) noexcept
	{
		if ((w * (rho - sigma * w / (v0 + kappa * theta * t))) >= T(0.0)) return T(0.0);
		else return std::copysign(std::numbers::pi_v<T> / T(T(12.0)), w);
	}

	template <std::floating_point T, std::size_t N>
	Complex<T> Pricer<T, N>::charFunction(T kappa,
		T theta,
		T sigma,
		T rho,
		T v0,
		...
		const Complex<T>& u) noexcept
	{
		...
		const Complex<T> r
		{
			(std::real(beta * std::conj(D)) > T(0.0))
			? -sigma2 * uu / betaPlusD
			: beta - D
		};
		...
		const Complex<T> A{(kappa * theta / sigma2) * (r * t - T(2) * math::log1pComplex<T>(-ry))	};
	...
	template <std::floating_point T, std::size_t N>
	Pricer<T, N>::CharFunCache Pricer<T, N>::charFunctionCal(T kappa,
		T theta,
		T sigma,
		T rho,
		T v0,
		...
		const Complex<T>& u) noexcept
	{
		...
		const Complex<T> y
		{
			(std::norm(D) > std::numeric_limits<T>::epsilon() *
							  (T(1) + std::abs(DT)))
			? math::expm1Complex(-DT) / (T(2) * D)
			: Complex<T>(-t / T(2))
		};
