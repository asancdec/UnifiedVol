...
{
/**
 * @brief Heston call option pricer using a damped Fourier / contour-shift
 * representation.
 *
 * Uses a Tanh-Sinh quadrature backend (provided externally) to evaluate the
 * Andersen–Lake contour-deformation integral formulation. @cite
 * AndersenLake2018
 *
...
template <std::floating_point T, std::size_t N> class Pricer
{
  private:
    ...
    /// @cond INTERNAL
    /**
     * @brief Cached characteristic-function intermediates for fast gradient
     * evaluation.
     *
    ...
    std::shared_ptr<const math::integration::TanHSinH<T, N>>
        quad_;               ///< Quadrature engine (shared).
    ...
    /**
     * @brief Heston characteristic function (Albrecher et al., 2007 stable
     * branch).
     *
     * @param kappa Mean reversion speed.
     * @param theta Long-run variance.
  ...
  public:
    ...
    /**
     * @brief Construct a pricer with a provided quadrature engine and
     * configuration.
