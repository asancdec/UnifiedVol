...
namespace uv
{
        ...
        unsigned int maxEval) noexcept
    {
        ...
        for (unsigned int i = 0U; i < maxEval; ++i)
        {
            ...
            const long double priceBS{ uv::blackScholes(t_, r_, q_, vol, S_, K_, isCall) };
            ...
            // Calculate d1
            const long double d1{ uv::d1BS(t_, r_, q_, vol, S_, K_) };

            // Calculate first derivative
            const long double vega{ uv::vegaBS(d1, t_, q_, S_) };

            // Calculate second derivative
            const long double volga{ uv::volgaBS(vega, d1, t_, vol) };
        ...
        // Calculate residual
        const long double objEval{ uv::blackScholes(t_, r_, q_, vol, S_, K_, isCall) - mktPriceBS_ };
